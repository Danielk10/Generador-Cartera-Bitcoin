...
Esta guía explica cómo utilizar el script `upload_play_store.py` para publicar actualizaciones automáticamente en la Google Play Store para el proyecto **Generador de Cartera Bitcoin (com.diamon.ganar)**.
...
- Actualización de librerías del sistema y dependencias de seguridad criptográfica.
- Mejoras internas de rendimiento, optimización y estabilidad." \
  --release_notes_en "- Updated to Android SDK 37 (Android 17) with improved compatibility.
- Updated system libraries and cryptographic security dependencies.
- Internal performance, optimization and stability improvements."
```
...
| :--- | :--- | :--- |
| `--package_name` | ID de la aplicación en Google Play | `com.diamon.ganar` |
...
```
El archivo de salida se generará en `/tmp/calculo/outputs/bundle/release/app-release.aab` firmado con las credenciales de `keystore.properties` (`/ruta/a/tu/keystore.jks` con alias `tu_alias`).

---

## 💡 Notas de Seguridad

- Los archivos `.jks`, `.keystore`, `keystore.jks`, `.json` de cuentas de servicio y `keystore.properties` están estrictamente excluidos en `.gitignore` para evitar cualquier filtración de claves privadas o credenciales a GitHub.
